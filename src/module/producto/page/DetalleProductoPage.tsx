...
import type { CarritoI, ProductoDetalle } from "../interface/producto";
...
export const ProductoDetallePage = () => {
    ...
    const [productoSeleccionado, setProductoSeleccionado] = useState<CarritoI | null>(null);
    ...
    useEffect(() => {
        if (productoSeleccionado) {
            setProductoSeleccionado((prev) => ({
                ...prev!,
                cantidad: cantidad,
                total: prev!.precioVenta * cantidad
            }));
        }
    }, [cantidad]);

    const handleWhatsApp = () => {
        const mensaje = `Hola! Estoy interesado en el producto: ${productoDetalle?.nombre}${productoSeleccionado?.talla ? ` (Talla: ${productoSeleccionado.talla})` : ""}.`;
    ...
    const btnCarrito = () => {
        ...
        const carritoActual = JSON.parse(localStorage.getItem("carrito_ropa") || "[]");
        ...
        if (existeIndex !== -1) {
            carritoActual[existeIndex].cantidad += productoSeleccionado.cantidad;
            carritoActual[existeIndex].total = carritoActual[existeIndex].cantidad * carritoActual[existeIndex].precioVenta;
        } else {
                    ...
                    <div className="grid lg:grid-cols-12 gap-12">
                                ...
                                {productoDetalle.imagenes.map((img, i) => (
                                    <button
                                        ...
                                        className={`relative flex-shrink-0 w-20 h-24 rounded-lg overflow-hidden border-2 transition-all ${
                                            imagenSeleccionada === img.nombre ? "border-black shadow-md" : "border-transparent opacity-60 hover:opacity-100"
                                        }`}
                                    >
                                        <img src={`${urlImagen}/${img.nombre}`} className="w-full h-full object-cover" alt="Miniatura" />
                                    </button>
                                ))}
                            </div>
                        ...
                        <div className="lg:col-span-5 flex flex-col">
                            ...
                            <div className="py-6">
                                ...
                                <p className="text-zinc-600 leading-relaxed text-[15px]">{productoDetalle.descripcion}</p>
                            </div>
                                        ...
                                        <button
                                            ...
                                            onClick={() => {
                                                setProductoSeleccionado({
                                                    ...
                                                    precioVenta: productoDetalle.precioVenta,
                                                    cantidad: cantidad,
                                                    talla: item.talla,
                                                    idStock: item.idStock,
                                                    total: productoDetalle.precioVenta * cantidad,
                                                    imagen: imagenSeleccionada
                                                });
                                            }}
                                            className={`px-5 py-2.5 rounded-xl border-2 text-sm font-bold transition-all ${
                                                productoSeleccionado?.talla === item.talla
                            ...
                            <div className="mt-10 flex flex-col gap-3">
                                <button
                                    ...
                                    onClick={btnCarrito}
                                    className={`w-full py-4 rounded-2xl flex items-center justify-center gap-2 font-bold transition-all shadow-xl active:scale-95 ${
